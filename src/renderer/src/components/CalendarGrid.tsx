...
import { BOOKING_COLORS, WEEKDAY_SHORT } from "@shared/defaults";
import {
  combineDateAndTime,
  formatDuration,
  formatTime,
...
function layoutDay(items: Booking[]): LaidOutBooking[] {
  ...
  for (const booking of sorted) {
    ...
    const endMinutes =
      startMinutes + minutesBetween(booking.start, booking.end);
    if (cluster.length > 0 && startMinutes >= clusterEnd) flush();

    const usedLanes = new Set(
      cluster
        .filter((item) => item.endMinutes > startMinutes)
        .map((item) => item.lane),
    );
    let lane = 0;
...
export default function CalendarGrid({
  ...
  const endHour = Math.max(
    startHour + 1,
    Math.min(24, settings.calendarEndHour),
  );
  ...
  const bookingsByDay = useMemo(() => {
    ...
    for (const day of days) {
      map.set(
        day,
        layoutDay(bookings.filter((booking) => booking.date === day)),
      );
    }
    return map;
  }, [days, bookings]);
  ...
  const officeStartMinutes = officeStart
    ? minutesSinceMidnight(officeStart)
    : 8 * 60;
  const officeEndMinutes = officeEnd
    ? minutesSinceMidnight(officeEnd)
    : 17 * 60;
  ...
  useEffect(() => {
    ...
    const handleUp = (): void => {
      ...
      const start = combineDateAndTime(
        dayKey,
        minutesToHhMm(current.startMinutes),
      );
      ...
      >
        {days.map((day) => {
          ...
          return (
            ...
            >
              <Caption1>{WEEKDAY_SHORT[isoWeekday(date) - 1]}</Caption1>
        ...
        {days.map((day, dayIndex) => {
          ...
          return (
            <div
              ...
              className={mergeClasses(
                styles.column,
                !isWorkday && styles.columnWeekend,
              )}
            ...
            >
              ...
              {slot < 60
                ? Array.from(
                    { length: Math.floor(totalMinutes / slot) },
                    (_, index) => (
                      <div
                        key={index}
              ...
              <div
                ...
                style={{
                  ...
                  height: Math.max(
                    0,
                    (officeEndMinutes - officeStartMinutes) * pxPerMinute,
                  ),
                }}
              />
              ...
              {laidOut.map((item) => {
                const dragging =
                  drag?.bookingId === item.booking.id && drag.moved;
                const top =
                  dragging && drag.dayIndex === dayIndex
                    ? drag.startMinutes
                    : item.startMinutes;
                const bottom =
                  dragging && drag.dayIndex === dayIndex
                ...
                const palette =
                  BOOKING_COLORS[item.booking.color] ?? BOOKING_COLORS.brand;
                ...
                return (
                  <div
                    ...
                    className={mergeClasses(
                      styles.booking,
                      settings.warnBookingOutsideWorkingHours &&
                        !covered &&
                        styles.outside,
                    )}
                    style={{
                    ...
                    onPointerDown={(event) =>
                      beginMove(event, item, dayIndex, "move")
                    }
                    role="button"
                    tabIndex={0}
                    onKeyDown={(event) => {
                  ...
                  >
                    ...
                    <span className={styles.bookingMeta}>
                      {formatTime(item.booking.start)}–
                      {formatTime(item.booking.end)} ·{" "}
                      {formatDuration(
                        minutesBetween(item.booking.start, item.booking.end),
                      )}
                    </span>
                    {bottom - top >= 45 ? (
                      <span className={styles.bookingMeta}>
                    ...
                    <div
                      ...
                      onPointerDown={(event) =>
                        beginMove(event, item, dayIndex, "resize")
                      }
                    />
                  </div>
                );
              ...
              {drag && drag.mode === "create" && drag.dayIndex === dayIndex ? (
                <div
                  ...
                  style={{
                    ...
                    height: Math.max(
                      14,
                      Math.abs(drag.endMinutes - drag.startMinutes) *
                        pxPerMinute,
                    ),
                    left: "2px",
                    right: "4px",
...
function coveredByWork(
  ...
  const covered = timeline.work.reduce(
    (total, segment) =>
      total +
      Math.max(0, Math.min(segment.end, end) - Math.max(segment.start, start)),
    0,
  );
